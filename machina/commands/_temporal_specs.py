...
``TEMPORAL_BACKEND`` env var:
  ...
  - ``postgres`` (prod) — two specs running the supervised runtimes:
    ...
    via ``machina.commands._supervised_runtime``.
...
from machina.supervisor import RestartPolicy, ServiceSpec
...
def _runtime_argv(factory_path: str) -> list[str]:
    ...
    return [
        "uv", "run", "python", "-m",
        "machina.commands._supervised_runtime",
        factory_path,
    ]
...
def temporal_specs(root: Path, cfg) -> list[ServiceSpec]:
    ...
    ``cfg`` is ``machina.config.Config``; ``cfg.temporal_port`` drives
