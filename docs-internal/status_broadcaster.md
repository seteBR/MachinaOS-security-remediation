...
| `agent_progress` | Per-turn LangGraph / AgentWorkflow progress (canvas badge "N / max") — wraps `WorkflowEvent.agent_progress` (CloudEvents v1.0). Emitted from inside `services/ai.py:execute_agent` (legacy path) and via the `agent.broadcast_progress.v1` Temporal activity (F4.B AgentWorkflow path). | `{specversion: "1.0", id, source: "machinaos://services/agent", type: "com.machinaos.agent.progress", subject: node_id, time, data: {node_id, iteration, max_iterations, phase?}}` |
