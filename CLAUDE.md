...
**Skill materialisation — shared helper, both paths.** [`services/cli_agent/_skills.py::materialise_skills`](./server/services/cli_agent/_skills.py) writes connected SKILL.md trees under `<cwd>/.claude/skills/<name>/` so claude's built-in `Skill` tool can load them. Filesystem skills are copied wholesale (`shutil.copytree`) preserving `scripts/` + `references/`; DB skills reconstruct the frontmatter from the loader's metadata. Called from BOTH `AICliSession._pre_spawn` (non-pool path) and `ClaudeSessionPool._spawn` (pool path) — closes the pre-existing gap where memory-bound runs got skills via MCP only. The helper is paired with the conditional `Skill` entry in `--allowedTools`: both fire iff `connected_skill_names` is non-empty.
