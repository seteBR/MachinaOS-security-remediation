...
- **`core.paths` is the SSOT for on-disk locations**: everything MachinaOs owns lives under `machina_root()` (= `~/.machina/` per default `DATA_DIR`). Subtree map: `workspaces/<slug>/` (per-workflow scratch), `daemons/` (supervised event-source daemon cwds — `stripe listen` etc.; shared root by default, no per-namespace subdir), `claude/` (Claude Code's `CLAUDE_CONFIG_DIR` state + `claude/npm/` binary install via `claude_npm_dir()`), `packages/<service>/` (downloaded binaries via `package_dir(name)` — `stripe`, `browser`, `temporal`), `whatsapp/` (WhatsApp session DB), `credentials.db` / `workflow.db` / `temporal.db`. Pre-fix `packages_dir()` routed through `platformdirs.user_cache_path("MachinaOs")` (`~/.cache/MachinaOs/` etc.) and the Temporal CLI sat in its own `pooch.os_cache("machinaos-temporal")` namespace — operators reported both as "not local" and the binary-vs-state split bought nothing in practice. Daemon cwds also used to live under `workspaces/_<namespace>/` and polluted per-workflow scratch with framework state. Out of scope: globally-installed binaries (Himalaya) and npm `package.json` deps (`edgymeow`). Shipped seed workflows are the lone exception — `example_workflows_dir()` is hardcoded to `<repo>/.machina/workflows/` (git-tracked seeds, NOT under `DATA_DIR`) so they survive `machina clean` and stay at the same path across `DATA_DIR=~/.machina` vs `DATA_DIR=.machina` configs.
